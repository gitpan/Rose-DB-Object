...
my $get_object = sub
{
  ...
  else
  {
    ...
    {
      $save_object->(${"${class}::Objects_By_Key"}{$key_name}{$key_value});
...
sub load
{
  ...
  unless(delete $args{'refresh'})
  {
    ...
    if(my $object = $get_object->($_[0], $pk))
    {
      $_[0] = $object;
      $_[0]->{STATE_IN_DB()} = 1;
      return 1;
    ...
    else
    {
      foreach my $cols ($_[0]->meta->unique_keys)
      {
        ...
        if(my $object = $get_object->($_[0], $key_name, $key_value))
        {
          $_[0] = $object;
          $_[0]->{STATE_IN_DB()} = 1;
          return 1;
...
The memory cache can be cleared for an individual object or all objects of the same class.  There is no automatic cache expiration, however.  You must clear the cache manually as you see fit.
...
This method does the same thing as the C<Rose::DB::Object> method of the same name, except that it also saves the object to the memory cache if the save succeeds.  If it fails, the memory cache is not modified.

=back

=head1 RESERVED METHODS

In addition to the reserved methods listed in the C<Rose::DB::Object> documentation, the following method names are also reserved for objects that inherit from this class:
